# Modified Software Requirement Panel
# June 2021
...
class Ui_OutputDialog(QDialog):
    ...
    @pyqtSlot()
    def startVideo(self, camera_name):  #turns on thne camera
        """
        ...
        path = 'ImagesAttendance' #includes the images from the file
        ...
        for cl in attendance_list: #splits the name and extension of the image
            cur_img = cv2.imread(f'{path}/{cl}')
            images.append(cur_img)
            self.class_names.append(os.path.splitext(cl)[0])
        for img in images: #sets cursor on the face
    ...
    def face_rec_(self, frame, encode_list_known, class_names): #self is used for attaching backend to frontend
        """
        ...
        def mark_attendance(name):#marks the attendence by finding time of system
            """
